...
function writenormQuat(bs, value)
...
function decompress_health_and_armor(hpAp)
...
function readCompressedVector(bs)
    local magnitude = bs:readFloat()
    if magnitude ~= 0 then
        local readCf = readCompressedFloat
...
function writeCompressedVector(bs, data)
	local x, y, z = data.x, data.y, data.z
    local magnitude = math.sqrt(x * x + y * y + z * z)
    bs:writeFloat(magnitude)
    if magnitude > 0 then
        local writeCf = writeCompressedFloat
